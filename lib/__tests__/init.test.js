...
describe('Init SDK', () => {
  ...
  it('should throw an explicit error when organization data can not be fetched', async () => {
  ...
  it('should throw an error if the fetched organization uses Azure Storage as media provider and no azure token is passed when initializing the SDK', async () => {
    const mockOrganization = {
      _id: 'myorg',
      name: 'Mock Rever Org',
      mediaProvider: config.mediaProviders.externalAzureStorage,
    };
  ...
  it('should use the default upload timeout from config when no timeout is specified in init args', async () => {
    const mockOrganization = { _id: 'myorg', name: 'Mock Rever Org' };

    jest.spyOn(axios, 'get').mockReturnValueOnce(Promise.resolve({ data: mockOrganization }));

    const reverMedia = await init(baseArgs);

    expect(reverMedia.uploadTimeout).toBe(config.upload.timeout);
  });

  it('should use the timeout provided in init args instead of the default', async () => {
    const mockOrganization = { _id: 'myorg', name: 'Mock Rever Org' };
    const customUploadTimeout = 120000;

    jest.spyOn(axios, 'get').mockReturnValueOnce(Promise.resolve({ data: mockOrganization }));

    const reverMedia = await init({ ...baseArgs, uploadTimeout: customUploadTimeout });

    expect(reverMedia.uploadTimeout).toBe(customUploadTimeout);
  });
});
