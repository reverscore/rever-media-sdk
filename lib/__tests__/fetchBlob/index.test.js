...
describe('Fetch BLOB', () => {
  it('should throw an error if media provider can not be determined', async () => {
    const args = {
      ...
      reverMediaObject: {
        ...
        provider: 'I_DONT_EVEN_EXIST',
      },
    };

    await expect(fetchBlob(args)).rejects.toThrow(
      `there is no fetching strategy for the "${args.reverMediaObject.provider}" media provider.`,
    );
  });
  ...
  it('should throw an error if Rever token is not provided when media provider is "EXTERNAL_MINIO"', async () => {
    const args = {
      ...omit(baseArgs, 'reverToken'),
      reverMediaObject: {
        ...baseArgs.reverMediaObject,
        provider: 'EXTERNAL_MINIO',
      },
    };
    await expect(fetchBlob(args)).rejects.toThrow(`"reverToken" param is required.`);
  });
  ...
  it('should call the Rever API fetcher with the expected args when organizations\' media provider is "EXTERNAL_MINIO"', async () => {
    const args = {
      ...baseArgs,
      reverMediaObject: {
        ...baseArgs.reverMediaObject,
  ...
  it('should include the "size" parameter in the resource url when the provider is AWS_S3 and the size is requested', async () => {
    await fetchBlob({
      ...baseArgs,
      reverMediaObject: {
        ...baseArgs.reverMediaObject,
  ...
  it('should include the "size" parameter in the resource url when the provider is EXTERNAL_MINIO and the size is requested', async () => {
    await fetchBlob({
      ...baseArgs,
      reverMediaObject: {
        ...baseArgs.reverMediaObject,
  ...
  it('should not include the "size" parameter in the resource url when the provider is AWS_S3 and the size is not requested', async () => {
    await fetchBlob({
  ...
  it('should not include the "size" parameter in the resource url when the provider is EXTERNAL_MINIO and the size is not requested', async () => {
    await fetchBlob({
      ...omit(baseArgs, 'options'),
      reverMediaObject: {
        ...baseArgs.reverMediaObject,
  ...
  it('should not include the "size" parameter in the resource url when the provider is EXTERNAL_AZURE_STORAGE_SERVICE and the size is not requested', async () => {
    await fetchBlob({
      ...baseArgs,
      reverMediaObject: {
        ...baseArgs.reverMediaObject,
  ...
  it('should not include the "size" parameter in the resource url when the provider is EXTERNAL_AZURE_STORAGE_SERVICE and the size is requested', async () => {
    await fetchBlob({
      ...baseArgs,
      reverMediaObject: {
        ...baseArgs.reverMediaObject,
