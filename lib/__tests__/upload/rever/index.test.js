...
import * as buildReverRequestPayload from '../../../upload/rever/buildRequestPayload';
...
describe('Upload to Rever', () => {
  ...
  it('should avoid forcing content-type when sending FormData so React Native Android can build the multipart boundary', async () => {
    axios.post.mockReturnValueOnce(Promise.resolve({ data: {} }));
  ...
  it('should call the payload generator utility when a file path is received', async () => {
    ...
    expect(buildReverRequestPayload.default).toHaveBeenCalledWith(args);
  });
});
