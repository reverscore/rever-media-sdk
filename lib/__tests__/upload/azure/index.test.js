...
describe('Upload to Azure Storage', () => {
  ...
  it('should call the corresponding Rever API endpoint with the proper params', async () => {
    ...
    const expectedRequestConfig = {
      ...
      timeout: baseArgs.uploadTimeout,
    };
  ...
  it('re-throws unauthorized errors from the Rever media object creation as-is', async () => {
    const unauthorizedError = new Error('Request failed with status code 401');
    unauthorizedError.response = { status: 401 };

    uploadForWeb.mockReturnValueOnce(Promise.resolve());
    axios.post.mockReturnValueOnce(Promise.reject(unauthorizedError));

    await expect(uploadToAzure(baseArgs)).rejects.toBe(unauthorizedError);
  });
});
