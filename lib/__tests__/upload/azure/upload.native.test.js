import { mockFetch, mockConfig } from 'react-native-blob-util';
...
describe('Upload to Azure from React Native clients', () => {
  ...
  it('should pass the uploadTimeout to ReactNativeBlobUtil.config when uploadTimeout is provided', async () => {
    mockFetch.mockReturnValueOnce(Promise.resolve({ data: {} }));
    await uploadForReactNative(baseArgs);

    expect(mockConfig).toHaveBeenCalledWith({ timeout: baseArgs.uploadTimeout });
  });

  it('should pass undefined timeout to ReactNativeBlobUtil.config when uploadTimeout is not provided', async () => {
    mockFetch.mockReturnValueOnce(Promise.resolve({ data: {} }));
    const argsWithoutTimeout = { ...baseArgs, uploadTimeout: undefined };

    await uploadForReactNative(argsWithoutTimeout);

    expect(mockConfig).toHaveBeenCalledWith({ timeout: undefined });
  });
});
